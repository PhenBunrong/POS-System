...
trait UploadFile
{
    ...
    public function updateOrInsertMorphOne($file_field, $model)
    {
        ...
        foreach($image_fillable as $file_field)
        {
            ...
            if (request()->has($file_field))
            {
                foreach(request()->$file_field as $img)
                {
                    ...
                    if($file['file_name'])
                    {
                        $model->files()->updateOrCreate([
                            'id'=> optional($model->files())->id,
                        ],[
                            'url'       => $file['file_name'], 
                            'type'      => $file_field, 
                            'mime_type' => $file['mime_type']
    ...
    public function base64UploadFile($base64_image_req, $with_mime_type = false)
    {
        ...
        if ($base64_image_req) :
            @list(, $file_data) = explode(';base64,', $base64_image_req);
            ...
            $mime = $base64_image_req->getClientOriginalExtension();
            // generate file name after decode
            $file_name = md5($base64_image_req . time()) . '.' . "$mime";

            // Locattion Store File
            $base64_image_req->move('uploads/files/',$file_name);
